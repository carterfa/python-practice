...
def write_grayscale(filename, pixels):
    """Creates and wrties a grayscale BMP file.
    ...
    with open(filename, 'wb') as bmp:
        ...
        # The next four bytes hold the filesize as a 32-bit little-endian integer. Zero placeholder for now
        size_bookmark = bmp.tell()
        bmp.write(b'\x00\x00\x00\x00')

        bmp.write(b'x\00x\00')  # Unused 16-bit integer - should be zero
        bmp.write(b'x\00x\00')  # Unused 16-bit integer - should be zero

        # The next four bytes hold the integer offset to the pixel data. Zero placeholder for now.
        ...
        bmp.write(b'\x08\x00')  # Bits per pizel 8 for grayscale
        ...
        # Fill in pixel offset placehoder
